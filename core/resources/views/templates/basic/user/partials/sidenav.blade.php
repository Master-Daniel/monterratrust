...
@endphp
<div class="col-lg-3">
    <div class="custom--card mb-4" style="background-color: #002046; color: white">
        <div class="card-body">
            <h6 class="mb-4 font-size--16px" style="color: white">{{ __(gs('site_name')) }} @lang('Balance')</h6>
            <h2 class="fw-normal main-balance main__amount__responsive" style="color: white">
                {{ showAmount(array_sum(@$totalBalance), gs('currency')) }}<sup>*</sup></h2>
            <div class="d-flex flex-wrap align-items-center justify-content-between mt-3">
                <p class="text-muted" style="color: white !important">@lang('Available')</p>
            ...
            @if (module('transfer_money', $module)->status)
                <a href="{{ route('user.withdraw') }}" class="btn btn--base btn-sm d-block mt-4">Withdraw</a>
            @endif
        </div>
    </div><!-- custom--card end -->
        ...
        <div class="card-body">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between mt-4">
                @if (module('request_money', $module)->status)
                    <a href="{{ route('user.request.money') }}" class="font-size--14px fw-bold">Loan</a>
                @endif
                <a href="{{ route('user.transactions', ['type'=>'plus_trx']) }}"
