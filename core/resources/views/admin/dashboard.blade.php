...
@section('panel')
    ...
    <div class="row gy-4 mt-2">
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.agents.all') }}" icon="las la-users" title="Total Agents" value="{{ $widget['total_agents'] }}" bg="primary" />
        </div><!-- dashboard-w1 end -->
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.agents.active') }}" icon="las la-user-check" title="Active Agents" value="{{ $widget['verified_agents'] }}" bg="success" />
        </div><!-- dashboard-w1 end -->
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.agents.email.unverified') }}" icon="lar la-envelope" title="Email Unverified Agents" value="{{ $widget['email_unverified_agents'] }}" bg="danger" />
        </div><!-- dashboard-w1 end -->
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.agents.mobile.unverified') }}" icon="las la-comment-slash" title="Mobile Unverified Agents" value="{{ $widget['mobile_unverified_agents'] }}" bg="warning" />
        </div><!-- dashboard-w1 end -->
    </div><!-- row end-->

    <div class="row gy-4 mt-2">
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.merchants.all') }}" icon="las la-users" title="Total Merchants" value="{{ $widget['total_merchants'] }}" bg="primary" type="2" />
        </div><!-- dashboard-w1 end -->
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.merchants.active') }}" icon="las la-user-check" title="Active Merchants" value="{{ $widget['verified_merchants'] }}" bg="success" type="2" />
        </div><!-- dashboard-w1 end -->
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.merchants.email.unverified') }}" icon="lar la-envelope" title="Email Unverified Merchants" value="{{ $widget['email_unverified_merchants'] }}" bg="danger" type="2" />
        </div><!-- dashboard-w1 end -->
        <div class="col-xxl-3 col-sm-6">
            <x-widget style="7" link="{{ route('admin.merchants.mobile.unverified') }}" icon="las la-comment-slash" title="Mobile Unverified Merchants" value="{{ $widget['mobile_unverified_merchants'] }}" bg="warning" type="2" />
        </div><!-- dashboard-w1 end -->
    </div><!-- row end-->
    ...
    <div class="row gy-4 mt-2">
        ...
        <div class="col-xxl-4 col-sm-6">
            <x-widget
                style="3"
                link="{{ route('admin.setting.general') }}"
                icon="las la-wallet"
                title="Default Currency"
                value="{{__(gs('cur_text'))}}"
                color="white"
                bg="1"
            />
        </div><!-- dashboard-w1 end -->
    ...
    <div class="row mb-none-30 mt-30">
        ...
        <div class="col-xl-6 mb-30">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex flex-wrap justify-content-between">
                        <div class="">
                            <h5 class="card-title">@lang('Transactions Report')
                                <a href="{{route('admin.trx.detail.graph')}}" class="ms-2">@lang('More details')<i class="las la-arrow-right"></i></a>
                            </h5>
        ...
        <div class="col-xl-6 mb-30">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex flex-wrap justify-content-between">
                        <h5 class="card-title">@lang('Charge & Commission Report')</h5>

                        <div id="chargeComDatePicker" class="border p-1 cursor-pointer rounded">
                            <i class="la la-calendar"></i>&nbsp;
                            <span></span> <i class="la la-caret-down"></i>
                        </div>
                    </div>
                    <div id="chargeComChartArea"></div>
                </div>
            </div>
        </div>
        <div class="col-xl-6 mb-30">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex flex-wrap justify-content-between">
                        <h5 class="card-title">@lang('Registration Report')</h5>

                        <div id="regDatePicker" class="border p-1 cursor-pointer rounded">
                            <i class="la la-calendar"></i>&nbsp;
                            <span></span> <i class="la la-caret-down"></i>
                        </div>
                    </div>

                    <div id="registrationChartArea"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    @include('admin.partials.generate_cot')
@endsection

@push('breadcrumb-plugins')
    <button class="btn btn-outline--primary btn-sm" data-bs-toggle="modal" data-bs-target="#cotModal">
        <i class="las la-server"></i>@lang('Generate COT Code')
    </button>
...
@push('script')
    <script>
        ...
        let chargeComChart = lineChart(
            document.querySelector("#chargeComChartArea"),
            [{
                    name: "Charges",
                    data: []
                },
        ...
        let regChart = lineChart(
            document.querySelector("#registrationChartArea"),
            [{
        ...
        const chargeCommissionChart = (startDate, endDate) => {
            ...
            const url = @json(route('admin.chart.charge.commission'));
            ...
            $.get(url, data,
                function(data, status) {
                    if (status == 'success') {
                        ...
                        chargeComChart.updateSeries(data.data);
                        chargeComChart.updateOptions({
                            xaxis: {
                                categories: data.created_on,
                            }
                        });
        ...
        const registrationChart = (startDate, endDate) => {
            ...
            const url = @json(route('admin.chart.registration'));
            ...
            $.get(url, data,
                function(data, status) {
                    if (status == 'success') {
                        ...
                        regChart.updateSeries(data.data);
        ...
        $('#chargeComDatePicker').daterangepicker(dateRangeOptions, (start, end) => changeDatePickerText('#chargeComDatePicker span', start, end));
        $('#regDatePicker').daterangepicker(dateRangeOptions, (start, end) => changeDatePickerText('#regDatePicker span', start, end));

        changeDatePickerText('#dwDatePicker span', start, end);
        changeDatePickerText('#trxDatePicker span', start, end);
        changeDatePickerText('#chargeComDatePicker span', start, end);
        changeDatePickerText('#regDatePicker span', start, end);

        depositWithdrawChart(start, end);
        transactionChart(start, end);
        chargeCommissionChart(start, end);
        registrationChart(start, end);
        ...
        $('#chargeComDatePicker').on('apply.daterangepicker', (event, picker) => chargeCommissionChart(picker.startDate, picker.endDate));
        $('#regDatePicker').on('apply.daterangepicker', (event, picker) => registrationChart(picker.startDate, picker.endDate));
